...
from resources.content import ContentTypes
...
class Project:
    ...
    def __init__(self, width: int, height: int, filename=None, *args, **kwargs) -> None:
        ...
        self.layout = Layout(
            x=kwargs.get("x", 0), y=kwargs.get("y", 0), width=width, height=height
        )
    ...
    def save(self, filename=None) -> None:
        ...
        with zipfile.ZipFile(
            output_file, "w", zipfile.ZIP_STORED, allowZip64=False
        ) as zf:
            print(zf.filename)
    ...
    def __enter__(self) -> "Project":
        ...
        if not self.filename:
            raise ValueError(
                "A filename must be provided for the project before entering the context manager."
            )
        return self
